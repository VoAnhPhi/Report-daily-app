...
import { BugReopenedData } from '../queue/mail-job.types';

export function bugReopenedSubject(data: BugReopenedData): string {
  return `[${data.bugCode}] Lỗi được mở lại để xử lý — ACTA`;
}

export function BugReopenedEmail(props: BugReopenedData) {
  return (
    <Html lang="vi">
      <Head>
      ...
      <Preview>{`Lỗi ${props.bugCode} đã được mở lại`}</Preview>
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={content}>
            <Heading style={h1}>{`Lỗi ${props.bugCode} đã được mở lại`}</Heading>
            <Text style={text}>Xin chào,</Text>
            <Text style={text}>
              Báo lỗi <strong>{props.bugCode}</strong> đã được mở lại và đang được{' '}
              {props.assigneeNames.join(', ')} xử lý lại (người báo: {props.creatorName}).
            </Text>
            {props.bugLink ? (
              <>
                <Hr style={hr} />
...
export default BugReopenedEmail;
