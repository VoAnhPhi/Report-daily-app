...
import { BugAssignedData } from '../queue/mail-job.types';

export function bugAssignedSubject(data: BugAssignedData): string {
  return `[${data.bugCode}] Bạn được giao xử lý lỗi — ACTA`;
}

export function BugAssignedEmail(props: BugAssignedData) {
  return (
    <Html lang="vi">
      <Head>
      ...
      <Preview>{`Lỗi ${props.bugCode} đã được giao cho bạn xử lý`}</Preview>
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={content}>
            <Heading style={h1}>{`Lỗi ${props.bugCode} đã được giao`}</Heading>
            <Text style={text}>Xin chào,</Text>
            <Text style={text}>
              Báo lỗi <strong>{props.bugCode}</strong> vừa được giao cho{' '}
              {props.assigneeNames.join(', ')} xử lý (người báo: {props.creatorName}).
            </Text>
            <Hr style={hr} />
...
export default BugAssignedEmail;
...
const ctaSection = { margin: '20px 0 4px' };
const ctaButton = {
