...
export const HalfWalletSubmittedAdminEmail = ({
  adminName = 'Admin',
  userName = 'Người dùng',
  walletAddress = '0x...',
  userReason,
  ...
  <Html lang="vi">
    ...
    <Preview>Yêu cầu xác thực ví HALF mới từ {userName}</Preview>
    <Body style={main}>
      <Container style={container}>
        ...
        <Section style={content}>
          <Heading style={h1}>🔐 Yêu cầu xác thực ví HALF mới</Heading>
          <Text style={text}>Xin chào <strong>{adminName}</strong>,</Text>
          <Text style={text}>
            Người dùng <strong>{userName}</strong> vừa gửi yêu cầu xác thực ví HALF và đang chờ được xử lý.
          </Text>
          <Section style={infoBox}>
            ...
            <div style={row}>
              <Text style={label}>Địa chỉ ví:</Text>
              <Text style={{ ...value, fontFamily: 'monospace', wordBreak: 'break-all' as const }}>{walletAddress}</Text>
            </div>
            {userReason && (
              <div style={row}>
                <Text style={label}>Lý do:</Text>
                <Text style={value}>{userReason}</Text>
              </div>
            )}
          </Section>
...
export default HalfWalletSubmittedAdminEmail;
