...
interface TeamMembersAddedEmailProps {
  userName: string;
  teamName: string;
  role?: string;
}

export const TeamMembersAddedEmail = ({
  userName = 'Thành viên',
  teamName = 'Team',
  role = 'Thành viên',
}: TeamMembersAddedEmailProps) => {
  ...
  return (
    <Html lang="vi">
      ...
      <Preview>
        Bạn đã được thêm vào team {teamName} với vai trò {getRoleText(role)}
      </Preview>
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={content}>
            <Heading style={h1}>Chào mừng bạn đến với team! 🎉</Heading>
            ...
            <Text style={text}>
              Bạn đã được thêm vào team <strong>{teamName}</strong> với vai trò{' '}
              <strong>{getRoleText(role)}</strong>.
            </Text>

            <Text style={text}>
              Chúc mừng bạn đã trở thành một phần của team! Hãy cùng nhau làm
              việc và đạt được những mục tiêu chung.
            </Text>
