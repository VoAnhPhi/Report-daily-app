...
interface ReviewResultEmailProps {
  entityLabel: string;
  targetName: string;
...
export const ReviewResultEmail = ({
  entityLabel = 'mục',
  targetName = '',
  reviewType = 'create',
  action = 'approved',
  reviewerName = 'Người duyệt',
  reviewNote,
  reviewUrl = 'https://admin.acta.vn',
}: ReviewResultEmailProps) => {
  const isApproved = action === 'approved';
  const previewText = isApproved
    ? `Yêu cầu duyệt ${entityLabel} đã được phê duyệt`
    : `Yêu cầu duyệt ${entityLabel} đã bị từ chối`;
  const headingText = isApproved
    ? 'Yêu cầu đã được phê duyệt ✓'
    : 'Yêu cầu đã bị từ chối ✗';
  const actionLabel = isApproved ? 'phê duyệt' : 'từ chối';
  const reviewTypeLabel = reviewTypeLabels[reviewType];
      ...
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={content}>
            ...
            <Text style={text}>
              Yêu cầu{' '}
              <strong style={highlightText}>{reviewTypeLabel}</strong>{' '}
              {entityLabel}
              {targetName ? (
                <>
                  {' '}
              ...
              của bạn đã được{' '}
              <strong style={highlightText}>{reviewerName}</strong>{' '}
              {actionLabel}.
            </Text>

            {reviewNote && (
              <Text style={text}>
                <strong style={highlightText}>Ghi chú từ người duyệt:</strong>{' '}
                {reviewNote}
              </Text>
            )}
            ...
            <Text style={footer}>
              Bạn nhận được email này vì bạn đã gửi yêu cầu duyệt {entityLabel}{' '}
              trên hệ thống ACTA.
            </Text>
          </Section>
...
export default ReviewResultEmail;
